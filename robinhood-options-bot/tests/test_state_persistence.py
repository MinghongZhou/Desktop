...
def test_save_then_load_round_trips_broker_cash_only(tmp_path, risk_settings):
    ...
    save_state(path, broker, risk_engine)

    restored = load_state(path, risk_settings)
    assert restored is not None
    restored_broker, restored_risk_engine = restored
...
def test_save_then_load_round_trips_open_positions_and_halt_flags(tmp_path, risk_settings):
    ...
    restored_broker, restored_risk_engine = load_state(path, risk_settings)
...
def test_save_overwrites_previous_state(tmp_path, risk_settings):
    ...
    restored_broker, _ = load_state(path, risk_settings)
