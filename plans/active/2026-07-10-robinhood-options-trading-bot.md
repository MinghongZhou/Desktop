...
| 8 | Daily 30-DTE credit spreads (row 1) + an optional news-sentiment filter (VADER-scored real historical Alpaca headlines; blocks a directional spread only when same-day sentiment contradicts it by more than a threshold, iron condors untouched) | Options (SPY) | Same ~3yr window as row 1 (756 days, 2023–2026) | Filter fired exactly as designed (audited row-by-row: 4 signal-days flipped from `bear_call_spread`→`no_trade`), but **all 4 were already being rejected by the risk engine anyway** (3× `max_concurrent_positions` at cap, 1× delta cap) — final equity, CAGR, and Sharpe came out identical to row 1 to the cent | **Inconclusive, not negative.** The filter mechanism is correct and real (real historical point-in-time news via Alpaca's `/v1beta1/news`, confirmed available for years back), but this window never gave it a chance to be the deciding factor — every disagreement it would have acted on was already capacity-constrained for an unrelated reason. Would need either loosened position/delta caps during a validation-only run, or a longer/different window, to actually test whether it changes outcomes. |

**A known limitation of row 8's sentiment scoring:** VADER is a general-purpose lexicon, not finance-tuned — it scored "Company beats expectations, raises full-year outlook" as neutral (0.0), missing the financial connotation of "beat"/"raise" entirely. A finance-tuned lexicon (Loughran-McDonald) or model (FinBERT) is the natural upgrade if this overlay ever gets a real test.
